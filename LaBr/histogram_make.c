...
Double_t num_bins = 4096; //number of bins to read in from histograms
...
void load_sum_histograms(const char analysis_filepath[], char cal_filepath[], TH1F *hist[], Int_t source_count, Int_t num_cores) {
  ...
  TLaBr * labr = 0;
  if (analysis->FindBranch("TLaBr")) {
    analysis->SetBranchAddress("TLaBr", & labr);
    cout << "Creating LaBr" << endl;
  } else {
    cout << "Branch TLaBr not found, bad things will happen" << endl;
  }
  ...
  for (int i = 0; i < num_entries - 1; i++) {
    analysis->GetEntry(i);
    if(labr) {
      for(int j = 0; j < labr->GetMultiplicity(); j++) {
        TLaBrHit *labr_hit = labr->GetLaBrHit(j);
        hist[labr_hit->GetDetector()-1]->Fill(labr_hit->GetCharge());
      }
    }
    if (i % 10000 == 0) {
