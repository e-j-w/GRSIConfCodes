Int_t num_cores = 8; //number of cores in TIGRESS detector
...
void find_centroids(TH1F *hist[], Double_t energy[], Double_t energy_er[], Double_t lin_gains[], Double_t lin_offsets[],
  ...
  for (int i = 0; i < num_cores; i++) {
    ...
    for (int j = 0; j < num_peaks_used; j++) {
      ...
      TF1 *fit = new TF1(Form("fit %i-%i", i, j),"[0]*(exp(-((x-[1])^2/(2*[2]^2)))) + [3] + [4]*(0.5*(1-(ROOT::Math::erf(((x-[1])/([2]*2^(0.5)))))))",x_guess - 25, x_guess + 25);
      fit->SetParameters(y_guess, x_guess, 15, 14, 1);
      fit->SetParLimits(0, y_guess*0.8, y_guess*1.2); //area
      fit->SetParLimits(1, x_guess - 5, x_guess + 5); //centroid
      fit->SetParLimits(2, 1.0, 30); //sigma of gaussian distribution
...
void quad_energy() {
  ...
  quad_out << "float LaBr_nonlin[" << num_cores << "] = {" << quad_thirds[0];
  for (int i = 1; i < num_cores; i++) {
    quad_out << ", " << quad_thirds[i];
  }//for

  quad_out << "};" << endl;
  quad_out << "float LaBr_gain[" << num_cores << "] = {" << quad_gains[0];
  for (int i = 1; i < num_cores; i++) {
    quad_out << ", " << quad_gains[i];
  }//for
  quad_out << "};" << endl;
  quad_out << "float LaBr_offset[" << num_cores << "] = {" << quad_offsets[0];
